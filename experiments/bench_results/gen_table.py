...
def collect(d):
    ...
    for tag, label in MODELS:
        ...
        # 明细自洽性: 由 checks_ok 反算的约束通过数，应当等于 CSV 里的 instruction_acc。
        # 不等 = 明细与汇总不是同一次跑批（例如被旧口径的 jsonl 覆盖过），必须报出来。
        dc_pass = dc_tot = None
        if detail_i:
            oks = [b for r in detail_i for b in r.get("checks_ok", [])]
            if oks:
                dc_pass, dc_tot = sum(1 for x in oks if x), len(oks)
        out[tag] = {
            "label": label, "tag": tag,
...
def consistency_problems(res):
    """返回 (tag, csv值与明细反算不一致) 的列表。"""
    bad = []
    for tag, _label in MODELS:
        r = res.get(tag) or {}
        if r.get("instr") is None or r.get("dc_pass") is None:
            continue
        if r["dc_tot"] != r["n_ck"]:
            bad.append((tag, f"明细约束数 {r['dc_tot']} != CSV 约束数 {r['n_ck']}"))
            continue
        expect = round(r["instr"] * r["dc_tot"])
        if expect != r["dc_pass"]:
            bad.append((tag, f"明细反算通过数 {r['dc_pass']} != CSV 推算 {expect}"
                             f"（{r['instr']*100:.1f}%）——明细与汇总不是同一次跑批"))
    return bad
...
def write_md(d, res, stamp, manifest=None):
    ...
    bad = consistency_problems(res)
    if bad:
        L += ["> 🔴 **明细与汇总不一致（结果不可信，先修数据再看分数）**：", ""]
        for tag, why in bad:
            L.append(f">   - `{tag}`: {why}")
        L.append("")
    L += ["| 模型 | IFEval instruction-level | IFEval prompt-level | GSM8K | 状态 |",
          "|---|---|---|---|---|"]
    for tag, label in MODELS:
        r = res[tag]
...
.mono{font-family:ui-monospace,Consolas,monospace;font-size:12px;color:#4b5563;word-break:break-all}
    ...
    bad = consistency_problems(res)
    if bad:
        H.append("<div class='note' style='background:#fdecea;border-left-color:#c0392b'>"
                 "<b>🔴 明细与汇总不一致——结果不可信，先修数据再看分数</b><ul>"
                 + "".join(f"<li><span class='mono'>{esc(t)}</span>: {esc(w)}</li>" for t, w in bad)
                 + "</ul></div>")
    H += ["<h2>总分对比</h2>",
