...
def load_prompts():
    """从 ifeval_lite.py 里取 PROMPTS。用 importlib 按文件路径加载，
    避免 sys.path 里同时存在多份同名文件时导错对象。"""
    ...
    for p in cands:
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location("_ifeval_lite_for_analysis", p)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            return mod.PROMPTS
        except Exception as e:
            safe_print(f"!! 加载 {p} 失败: {e}")
    return None
